...
FROM
	...
	INNER JOIN (
		...
		GROUP BY
			sub_film_id
	) AS subq ON film.film_id = subq.sub_film_id
	AND subq.film_counter > 3
GROUP BY
	film.film_id
ORDER BY
	film.film_id ASC;
...
WHERE
	amount = (
		...
		FROM
			payment AS pt2
		WHERE
			pt1.customer_id = pt2.customer_id
	)
ORDER BY
	customer_id;
...
FROM
	payment AS p1
	INNER JOIN customer ON p1.customer_id = customer.customer_id
WHERE
	amount = (
		SELECT
			MAX(amount)
