...
--without a where clause; every row gets updated
...
DELETE FROM payment
WHERE
	payment_id IN (17064, 17067) RETURNING *;
...
DELETE FROM customer_mini RETURNING *;
...
-- VIEW
CREATE
OR REPLACE VIEW customer_spendings AS -- (or replace) is just so I can run whole file at once
SELECT
	initcap (first_name || ' ' || last_name) AS full_name,
...
-- 
ALTER VIEW customer_spendings
RENAME TO v_customer_spendings;

-- 
ALTER VIEW v_customer_spendings
RENAME COLUMN total_spend TO total_spending;

-- 
SELECT
	*
FROM
	v_customer_spendings;
...
-- MATERIALIZED VIEW
CREATE MATERIALIZED VIEW IF NOT EXISTS films_category AS
SELECT
	title,
	ct.name AS film_name,
