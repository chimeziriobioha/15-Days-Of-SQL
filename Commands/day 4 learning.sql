...
-- CONCATENATE with ||
SELECT
	initcap (first_name || ' ' || last_name) AS fullname
FROM
	customer
LIMIT
	20;
...
SELECT
	LEFT (email, 1) || '***@' || SPLIT_PART (email, '@', -1) AS anon_email
...
SELECT
	...
	SUBSTRING(
		email
		from
			POSITION('.' IN email) + 1 for POSITION('@' IN email) - POSITION('.' IN email) -1
	)
FROM
	customer
LIMIT
...
SELECT
	LEFT (email, 1) || '***' || LEFT (SUBSTRING(email, POSITION('.' IN email)), 2) || '***' || SUBSTRING(email, POSITION('@' IN email)) AS anonimised_email
FROM
	customer
LIMIT
	20;
...
SELECT
	...
	|| SUBSTRING(email, POSITION('.' IN email) -1, 3) || '***' || SUBSTRING(email, POSITION('@' IN email)) AS anonimised_email
FROM
	customer
LIMIT
	20;
...
SELECT
	...
	REPLACE (
		REPLACE (
			TO_CHAR (payment_date, 'Day, DDth Month, YYYY'),
			' ',
			''
		),
		',',
...
FROM
	film
WHERE
	rental_rate < ROUND(replacement_cost * 0.04, 2)
ORDER BY
	percentage ASC