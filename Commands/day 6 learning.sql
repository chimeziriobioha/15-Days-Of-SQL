...
FROM
	payment AS pa
	INNER JOIN customer AS cu ON pa.customer_id = cu.customer_id;
...
SELECT
	fare_conditions,
	COUNT(bps.seat_no) AS num_of_booking
FROM
	boarding_passes AS bps
	INNER JOIN seats AS sts ON bps.seat_no = sts.seat_no
	INNER JOIN flights AS flt ON bps.flight_id = flt.flight_id
...
SELECT
	fare_conditions,
	COUNT(*) AS num_of_booking
FROM
	boarding_passes AS bps,
	seats AS sts,
	flights AS flt
WHERE
	bps.seat_no = sts.seat_no
...
FROM
	tickets tks
	FULL JOIN boarding_passes bps ON tks.ticket_no = bps.ticket_no
WHERE
	bps.boarding_no IS null;

-- count of flights by each aircraft
SELECT
	acd.*,
	COUNT(fts.aircraft_code) AS flights_count
FROM
	aircrafts_data acd
	LEFT JOIN flights fts ON acd.aircraft_code = fts.aircraft_code
GROUP BY
	acd.aircraft_code
ORDER BY
	flights_count DESC;
...
SELECT
	*
FROM
	aircrafts_data AS acd
	FULL JOIN flights AS fts ON acd.aircraft_code = fts.aircraft_code
WHERE
	fts.flight_id IS null;
...
FROM
	aircrafts_data AS acd
	LEFT JOIN flights AS fts ON acd.aircraft_code = fts.aircraft_code
WHERE
	fts.flight_id IS null;
...
FROM
	seats AS sts
	LEFT JOIN boarding_passes AS bps ON sts.seat_no = bps.seat_no
GROUP BY
	seat_code
ORDER BY
	usage_count DESC;
...
FROM
	boarding_passes AS bps
	RIGHT JOIN seats AS sts ON sts.seat_no = bps.seat_no
GROUP BY
	seat_code
ORDER BY
	usage_count DESC;
...
FROM
	boarding_passes AS bps
	LEFT JOIN ticket_flights AS tfs ON bps.ticket_no = tfs.ticket_no
...
FROM
	tickets AS tks
	INNER JOIN ticket_flights AS tfs ON tks.ticket_no = tfs.ticket_no
	LEFT JOIN flights AS fts ON tfs.flight_id = fts.flight_id;
