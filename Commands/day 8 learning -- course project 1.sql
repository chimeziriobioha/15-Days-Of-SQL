...
SELECT
	first_name,
	last_name,
	COUNT(fa.actor_id) AS num_of_movies
FROM
	actor
...
SELECT
	ROUND(AVG(sunday_sum), 2) AS sunday_avearge
FROM
	(
		SELECT
			DATE(payment_date) as sunday,
			SUM(amount) as sunday_sum
		FROM
			payment
		WHERE
			EXTRACT(
...
SELECT
	...
	(
		SELECT
			ROUND(AVG(customer_total), 2) AS average_spend
		FROM
			(
				SELECT
					c2.customer_id,
		...
		WHERE
			a1.district = customer_totals.district
	)
FROM
	payment AS p1
...
SELECT
	...
	(
		...
		FROM
			category AS c1
			INNER JOIN film_category AS fc1 ON c1.category_id = fc1.category_id
